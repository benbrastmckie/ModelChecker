...
- **Task Type**: Task type (general|meta|markdown, plus extension types when loaded; auto-detected or user-provided)
...
- Task involves extension-specific work → use extension task type (e.g., from loaded extension)
...
- Task is file-specific (e.g., "Fix bug in module.ext")
...
- **Extension routing and MCP validation**:
  - Extension-specific research/implementation requests **must** use the extension-provided agents, selected from the TODO `Type` field (or explicit flag override) rather than heuristics.
  - Validate extension MCP server availability against `.mcp.json` before dispatch; fail with a clear error if required servers are missing/unreachable.
  - When validation fails, surface remediation steps (install command, set env variables, supply API keys) and do **not** create project directories or artifacts.
...
-   [ ] Language field reflects the primary language for the work (e.g., `general`, `meta`, `markdown`, or extension-provided types).
...
**Impact**: 
- Prevents proper routing to specialized agents (extension-provided agents, meta subagents)
...
1. Identify the primary language for the task:
   ...
   - Extension-specific tasks → Use extension-provided type (e.g., from loaded extension)
...
1. "Type field is required but could not be detected"
   - Solution: Add `--type general` (or markdown/meta) flag to /task command
   - Example: `/task "Fix bug in module" --type general`
...
1. "Task {number} missing required Type field"
   ...
   - Follow format: `- **Type**: {general|meta|markdown}` (or extension-provided type)
