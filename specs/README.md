...
2. **Slash Commands**: Use appropriate command for document type:
   - `/report` → creates in `research/`
   - `/debug` → creates in `debug/`
   - `/plan` → creates in `plans/`
   - `/implement` → creates summaries in `implementation/`
3. **Updating This README**: Add entry under appropriate section when feature is complete
4. **Cross-References**: Link between related documents using relative paths
5. **Important Findings**: Incorporate into main documentation, code comments, or tests
