# Implementation Plan: Fix Per-Example Solver Backend Leak

- **Task**: 73 - Fix per-example solver backend leaking across examples via _cli_override
- **Status**: [NOT STARTED]
- **Effort**: 0.5 hours
- **Dependencies**: None
- **Research Inputs**: reports/01_backend-leak-analysis.md
- **Artifacts**: plans/01_clear-cli-backend.md (this file)
...
- **Root cause identified**: `set_backend_with_invalidation()` sets `_cli_override` which never gets cleared
- **Recommended fix (Option A)**: Add `clear_cli_backend()` call in finally block of `runner.py`
- **Existing API**: `clear_cli_backend()` already exists in `solver/registry.py` (lines 81-84)
...
|------|--------|------------|------------|
| Clearing backend breaks CLI override behavior | High | Low | CLI override should only persist for initial setup, not across examples |
| Import adds overhead | Low | Low | Import is inside finally block, only happens once per example |
| Other code relies on sticky override | Medium | Low | Research showed no other consumers of sticky behavior |
...
**Tasks**:
- [ ] Edit `code/src/model_checker/builder/runner.py` line 765-767
- [ ] Add import and `clear_cli_backend()` call in the existing finally block

**Timing**: 15 minutes

**Files to modify**:
- `code/src/model_checker/builder/runner.py` - Add clear call in finally block after `process_example()`
...
- plans/01_clear-cli-backend.md (this file)
- summaries/01_clear-cli-backend-summary.md (after implementation)

## Rollback/Contingency

If the fix causes issues:
1. Remove the two added lines from `runner.py` (the import and `clear_cli_backend()` call)
