...
159 [NOT STARTED] — Withdraw the bimodal theory from the published release surface un

## Tasks

### 159. Withdraw bimodal from release surface
- **Status**: [NOT STARTED]
- **Task Type**: python
- **Topic**: release-engineering
- **Dependencies**: None

**Description**: Withdraw the bimodal theory from the published release surface until its solver-timing defects are genuinely fixed, then create a follow-up task to fix bimodal fully. Motivated by the v1.3.0 release run (2026-08-12): bimodal is the only theory with unresolved timing defects, and those defects are documented as NOT closable by budget tuning.

EVIDENCE, AND AN HONEST COUNTERWEIGHT THE FIRST PHASE MUST WEIGH BEFORE PROCEEDING. Two open defects, both bimodal-scoped, both shared-tenancy/solver-timing rather than semantic:
  - `test_bimodal.py::test_example_cases[BM_CM_1-example_case7]` fails intermittently. `examples.py`'s BM_CM_1_settings comment records a 15->60s recalibration and states a divergent draw measured undecided at 600s (~64x median), concluding "the divergent-draw residual is accepted and recorded: no budget closes it."
  - `oracle/bimodal_logic/tests/test_cross_oracle_differential.py::TestGatingConclusiveScan::test_known_conclusive_population_self_consistent` fails at 95-96 of 103 conclusive against floor=100, consistently, on GitHub runners; red on master since at least 2026-08-10.
COUNTERWEIGHT: the evidence also says bimodal WORKS. It shipped in 1.3.0; post-publish verification installed 1.3.0 from PyPI and ran bimodal successfully (exit 0, ~770 lines of genuine countermodel output), and BM_CM_1 finds its countermodel on every decided draw. The failures are a heavy-tailed Z3 search on one example plus a conclusive-population shortfall, not a semantic break. Withdrawing an entire working theory is therefore a heavy remedy for a flaky-test problem, and Phase 1 MUST put that trade-off to the user explicitly and get an answer before any removal lands. Alternatives to price against full withdrawal: (a) keep bimodal registered but mark it experimental/unstable in docs and CHANGELOG; (b) withdraw only BM_CM_1 from the release-gating example set while keeping the theory registered; (c) full withdrawal as specified below. Proceed to Phase 2+ only on an explicit decision; if the user picks (a) or (b), re-scope the remaining phases accordingly rather than forcing (c).

(1) DECIDE AND RECORD THE SCOPE (see above). Do not skip. Record the decision and its rationale in the task's summary so the follow-up task inherits it.

(2) MECHANISM -- WHAT WITHDRAWAL CONCRETELY MEANS. `code/src/model_checker/theory_lib/__init__.py:63`'s `_THEORY_NAMES` list is the single literal driving registration: `_register_theories()` iterates it into `registry.register_theory()`, and `AVAILABLE_THEORIES` (line 482) is a computed VIEW over the registry, not an independent list. Removing `'bimodal'` from `_THEORY_NAMES` is therefore the one edit that withdraws it from the registry, the CLI's `-l/--load_theory` surface, and `AVAILABLE_THEORIES` together. Do NOT reintroduce a second literal list; the file explicitly warns against that.

(3) FOLLOW THE BLAST RADIUS. Removing the name breaks every consumer that assumes four theories. At minimum, audit and update:
    - `code/tests/packaging/test_cli_console_script.py` and `test_generate_then_execute.py` (both assert over the theory set)
    - `code/src/model_checker/theory_lib/tests/test_theory_conformance.py` and `check_registry_drift()` -- an unregistered-but-present `bimodal/` directory is exactly the drift condition `discover_theories()` vs `AVAILABLE_THEORIES` reports; decide whether bimodal becomes an EXPECTED unregistered directory or is excluded from discovery too
    - `code/MANIFEST.in:22` (recursive-include of the bimodal README)
    - the bimodal test suite itself -- it should keep running on branches/PRs so the follow-up task has a signal to work against; withdrawal from the RELEASE is not withdrawal from CI
    - `.github/workflows/differential-tests.yml` -- reassess whether the oracle suite still gates anything once bimodal is unpublished (this overlaps the CI-hardening task's item 12; reconcile, do not duplicate)
    - user-facing docs and `code/CHANGELOG.md`

(4) THIS IS A USER-VISIBLE REMOVAL. bimodal SHIPPED in 1.3.0, so withdrawing it is a breaking change for anyone who installed 1.3.0 and used it. The project's stated policy (CLAUDE.md, "No Backwards Compatibility - clean breaks when improving code") supports a clean break over a deprecation shim, but the CHANGELOG entry must state the removal plainly, name the reason (unresolved solver-timing defects, not a semantic fault), and say that the theory remains in-tree and under repair rather than deleted. Decide the version number for the release carrying the removal.

(5) VERIFY THE WITHDRAWAL. After the change: `AVAILABLE_THEORIES` has three entries; `model-checker -l bimodal` fails with a clear "not registered" error naming the available theories rather than a traceback; a fresh `python -m build` + `bash code/scripts/release-verify.sh --ref 1.3.0` passes; the packaging contract suite passes; and the three remaining theories still generate-and-execute clean (3/3). Note that the golden-path check is currently NOT scriptable -- `project.py` prompts interactively -- so either drive it manually or land the non-interactive flag from the CI-hardening task first.

(6) TERMINAL DELIVERABLE -- CREATE THE FOLLOW-UP TASK. This task MUST conclude by creating a new task to fix the bimodal theory fully and restore it to the release surface. That follow-up task's description must carry forward, concretely: the BM_CM_1 divergent-draw evidence and the standing verdict that no budget closes it (so the follow-up does not waste a cycle re-tuning `max_time`); the oracle conclusive-population shortfall with its 95-96/103-vs-floor-100 measurements and the instruction NOT to lower the floor to reach green; the scope decision recorded in Phase 1; the exact set of files this task changed, so restoration is a reversal against a known list; and an explicit restoration acceptance bar -- what must be true for bimodal to be re-registered (e.g. BM_CM_1 decides within budget across N consecutive CI runs, and the oracle population meets its floor on CI hardware, not merely on a quiet host). Set the follow-up's `task_type` to `python` and give it a `file_scope` covering the bimodal theory package, its tests, and the oracle bimodal tree.

RELATIONSHIP TO THE CI-HARDENING TASK: that task's items (10) (harden/quarantine the BM_CM_1 flake), (11) (investigate the oracle floor), and (12) (decide whether the oracle suite gates anything) all address the SAME two defects from the CI side rather than the release side. These two tasks must be reconciled, not executed blindly in parallel: if bimodal is withdrawn here, item (10) narrows to branch-CI hygiene only, and items (11)-(12) should fold into the follow-up task created in Phase 6. Whichever runs second must re-read the other's outcome first. No dependency edge is set, because the CI task's headline TestPyPI-gate work is independent of bimodal and should not be stalled behind this decision.

AGENT CONSTRAINT: per .claude/rules/pr-prohibition.md, `git push`, `git tag`, `/merge`, `/tag`, and any twine upload remain USER-ONLY. This task prepares and verifies the withdrawal; it does not publish it.
