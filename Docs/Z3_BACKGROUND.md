...
1. **Function-based uninegation** (`\func_unineg`): Uses witness functions h and y (Champollion-Bernard semantics)
2. **Set-based uninegation** (`\set_unineg`): Uses set operations without functions (Fine semantics)

The challenge of implementing `\func_unineg` motivated the witness predicate architecture described in the exclusion theory documentation.