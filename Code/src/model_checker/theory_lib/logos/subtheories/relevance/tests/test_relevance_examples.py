...
relevance_registry.load_subtheories(['extensional', 'modal', 'constitutive', 'relevance'])

@pytest.mark.parametrize("example_name, example_case", relevance_examples.items())
def test_relevance_examples(example_name, example_case):
    ...
    result = run_test(
        ...
        LogosSemantics,
        LogosProposition,
        relevance_registry.get_operators(),
        Syntax,
        ModelConstraints,
        LogosModelStructure,
    )
