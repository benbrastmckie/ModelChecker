...
counterfactual_registry.load_subtheories(['extensional', 'counterfactual'])

@pytest.mark.parametrize("example_name, example_case", counterfactual_examples.items())
def test_counterfactual_examples(example_name, example_case):
    ...
    result = run_test(
        ...
        LogosSemantics,
        LogosProposition,
        counterfactual_registry.get_operators(),
        Syntax,
        ModelConstraints,
        LogosModelStructure,
    )
