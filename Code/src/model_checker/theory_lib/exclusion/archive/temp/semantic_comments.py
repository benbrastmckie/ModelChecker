...
class ExclusionSemantics(model.SemanticDefaults):
    ...
    def __init__(self, combined_settings): # TODO: 
        ...
        # QI
        # TODO: include comments to gloss definition
        # NOTE: shouldn't this be a function from integers (indecies) to functions/arrays'
        # instead it takes bitvectors as input
        ...
        # NOTE: I added this to get some slightly more classical behavior but fine to ignore
        null_state = ForAll(
            x,
    ...
    # would they be equivalent if the following constraint were added:
    # (CON_REF) if x and y are parts of s that exclude each other, then s excludes s
    # NOTE: (s coheres t) is: no part a of s and part b of t are s.t. a excludes b
    # (s compossible t) is: no parts x, y of s.t are s.t. x excludes y
    # even if both s and t are possible (no two parts of s/t exclude each other) 
    # and s coheres with t, then there could be parts x, y of s.t that exclude each other.
    # CON_REF could still fail to apply since x, y could overlap both s and t making
    # both fail to be parts of just s/t
    ...
    # TODO: should this be eval_point? (YES, but can wait)
