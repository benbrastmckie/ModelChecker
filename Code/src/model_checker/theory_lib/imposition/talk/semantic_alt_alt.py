...
class LogosSemantics(SemanticDefaults):
    ...
    def __init__(self, combined_settings=None, operator_registry=None, **kwargs):
        ...
        self.verify = z3.Function(
            "verify", z3.BitVecSort(self.N), syntactic.AtomSort, z3.BoolSort()
        )
        self.falsify = z3.Function(
            "falsify", z3.BitVecSort(self.N), syntactic.AtomSort, z3.BoolSort()
        )
        ...
        self.is_alternative = z3.Function(
            "is_alternative",
        ...
        u = z3.BitVec("alt_u", self.N)
        y = z3.BitVec("alt_y", self.N)
        w = z3.BitVec("alt_w", self.N)
        # return z3.And(
        #     self.is_world(u),
        #     self.is_part_of(y, u),
        #     Exists(z, z3.And(self.is_part_of(z, u), self.max_compatible_part(z, w, y))),
        # )
        is_alternative_constraint = ForAll(
            [u, y, w],
            z3.And(
                z3.Implies(
                    self.is_alternative(u, y, w),
                    z3.And(
                        self.is_world(u),
                        self.is_part_of(y, u),
                        Exists(
                            z,
                            z3.And(
                                self.is_part_of(z, u), self.max_compatible_part(z, w, y)
                            ),
                        ),
                    ),
                ),
        ...
        self.frame_constraints = [
            ...
            is_alternative_constraint,
        ]

        # Define invalidity conditions
        self.premise_behavior = lambda premise: self.true_at(premise, self.main_point)
        self.conclusion_behavior = lambda conclusion: self.false_at(
            conclusion, self.main_point
        )
    ...
    def true_at(self, sentence, eval_point):
        ...
        if sentence_letter is not None:
            ...
            return Exists(
                x,
                z3.And(self.is_part_of(x, eval_world), self.verify(x, sentence_letter)),
            )
    ...
    def false_at(self, sentence, eval_point):
        ...
        if sentence_letter is not None:
            ...
            return Exists(
                x,
                z3.And(
                    self.is_part_of(x, eval_world), self.falsify(x, sentence_letter)
                ),
            )
        operator = sentence.operator
        arguments = sentence.arguments or ()
...
class LogosProposition(PropositionDefaults):
    ...
    def __init__(self, sentence, model_structure, eval_world="main"):
        ...
        self.eval_world = (
            model_structure.main_point["world"] if eval_world == "main" else eval_world
        )
    ...
    def proposition_constraints(self, sentence_letter):
        ...
        def get_contingent_constraints():
            ...
            possible_verifier = Exists(
                x, z3.And(semantics.possible(x), semantics.verify(x, sentence_letter))
            )
            possible_falsifier = Exists(
                y, z3.And(semantics.possible(y), semantics.falsify(y, sentence_letter))
            )
            return [
                possible_verifier,
                possible_falsifier,
...
class LogosModelStructure(ModelDefaults):
    ...
    def print_all(
        self, default_settings, example_name, theory_name, output=sys.__stdout__
    ):
    ...
    def print_to(
        ...
        actual_timeout = (
            hasattr(self, "z3_model_runtime")
            and self.z3_model_runtime is not None
            and self.z3_model_runtime >= self.max_time
        )
        ...
        if actual_timeout and (not hasattr(self, "z3_model") or self.z3_model is None):
            print(
                f"\nTIMEOUT: Model search exceeded maximum time of {self.max_time} seconds",
                file=output,
            )
            print(
                f"No model for example {example_name} found before timeout.",
                file=output,
            )
    ...
    def print_model_differences(self, output=sys.stdout):
        ...
        # Print world changes
        if diffs.get("worlds", {}).get("added") or diffs.get("worlds", {}).get(
            "removed"
        ):
        ...
        if diffs.get("possible_states", {}).get("added") or diffs.get(
            "possible_states", {}
        ).get("removed"):
        ...
        if diffs.get("verify"):
            ...
            for letter, state_changes in diffs["verify"].items():
                ...
                for state_str, change in state_changes.items():
                    if change["new"]:
                        print(
                            f"    {GREEN}+ {state_str} now verifies {letter}{RESET}",
                            file=output,
                        )
                    else:
                        print(
                            f"    {RED}- {state_str} no longer verifies {letter}{RESET}",
                            file=output,
                        )
            print("", file=output)
        ...
        if diffs.get("falsify"):
            ...
            for letter, state_changes in diffs["falsify"].items():
                ...
                for state_str, change in state_changes.items():
                    if change["new"]:
                        print(
                            f"    {GREEN}+ {state_str} now falsifies {letter}{RESET}",
                            file=output,
                        )
                    else:
                        print(
                            f"    {RED}- {state_str} no longer falsifies {letter}{RESET}",
                            file=output,
                        )
            print("", file=output)
    ...
    def print_states(self, output=sys.__stdout__):
        ...
        def format_state(bin_rep, state, color, label=""):
            ...
            if use_colors:
                print(
                    f"  {self.WHITE}{bin_rep} = {color}{state}{label_str}{self.RESET}",
                    file=output,
                )
            else:
