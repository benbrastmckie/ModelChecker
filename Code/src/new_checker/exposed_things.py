...
class Semantics:
    ...
    def true_at(self, sentence_object, eval_world):
        """
        sentence_object is always a list, eval world a BitVector
        sentence_object is the third kind of sentence_object
        """
        if str(sentence_object[0]).isalnum():
            sentence_letter = sentence_object[0]
            x = z3.BitVec("t_atom_x", self.N)
            return Exists(x, z3.And(self.is_part_of(x, eval_world), self.verify(x, sentence_letter)))
        operator = sentence_object[0]
        assert not isinstance(operator, type), "operator should be an instance of a class"
        args = sentence_object[1:]
        return operator.true_at(*args, eval_world)

    def false_at(self, sentence_object, eval_world):
        if str(sentence_object[0]).isalnum():
            sentence_letter = sentence_object[0]
    ...
    def extended_verify(self, state, sentence_object, eval_world):
        if str(sentence_object[0]).isalnum():
            return self.verify(state, sentence_object[0])
        op, args = sentence_object[0], sentence_object[1:]
        return op.extended_verify(state, *args, eval_world)
    
    def extended_falsify(self, state, sentence_object, eval_world):
        if str(sentence_object[0]).isalnum():
            return self.falsify(state, sentence_object[0])
        op, args = sentence_object[0], sentence_object[1:]
...
class Proposition(PropositionDefaults):
    ...
    def __eq__(self, other):
        return (
            ...
            and str(self.sentence_object) == str(other.sentence_object)
        )
    ...
    def find_proposition(self):
        ...
        if self.arguments is None: # self.arguments is a list of Sentence objects
            ...
            if self.prefix_sentence[0] in {'\\top', '\\bot'}:
                operator = self.sentence_object[0]
            ...
            if self.prefix_sentence[0].isalnum():
                ...
                # about the idea that all sentence_objects are lists including
                # sentence letters. that still seems like a good convention
                # print("LETTER", self.sentence_object)
                sentence_letter = self.sentence_object[0]
                V = {
...
class AndOperator(syntactic.Operator):
    ...
    def true_at(self, leftarg, rightarg, eval_world):
        """doc string place holder
        args are sentence_objects (ie things of the third kind) I think, def 2nd or 3rd kind
        """
        sem = self.semantics
...
class CounterfactualOperator(syntactic.Operator):
    ...
    def find_verifiers_and_falsifiers(self, left_sent_obj, right_sent_obj, eval_world):
        ...
        # self.sentence_object = 
        leftarg, rightarg = left_sent_obj.sentence_object, right_sent_obj.sentence_object
