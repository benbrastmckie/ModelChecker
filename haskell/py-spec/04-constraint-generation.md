...
tree calling `update_objects` on every node — phase 3 of the `Sentence` lifecycle (see the
sentence lifecycle table), replacing operator classes with operator instances; then builds the
...
|---|---|---|
| `frame_constraints` | the theory semantics' own `__init__` | the theory's frame axioms — a **closed, exact list per theory** (see below), not an open-ended family |
| `model_constraints` | `proposition_class.proposition_constraints(mc, letter)`, once per sentence letter | the per-atom "what is a proposition" menu, settings-gated (see [`05-state-encoding.md`](./05-state-encoding.md)) |
...
## The frame-constraint lists are closed

Each theory asserts an exact, enumerable list of frame constraints — a port implements *this
list*, not a general axiom family. For **logos** the list has exactly two members:

1. **Possibility downward closure**: `∀x, y. (possible(y) ∧ x ⊑ y) → possible(x)`
2. **Actuality**: `is_world(main_world)` — the designated evaluation world is a world

(`is_world` and the helper predicates it is built from are defined in
[`05-state-encoding.md`](./05-state-encoding.md)'s helper-predicate table.) Nothing else is a
logos frame constraint; every other constraint in the model belongs to one of the other three
groups.

The other theories' lists are equally closed:

- **Imposition** asserts logos's two constraints plus Fine's four conditions on the primitive
  ternary `imposition` relation — *inclusion*, *actuality*, *incorporation*, *completeness* —
  stated as formulas in the operator semantics document
  ([`03a-operator-semantics.md`](./03a-operator-semantics.md)).
- **Exclusion** asserts four: actuality (`is_world(main_world)`), exclusion symmetry, harmony,
  and the rashomon principle; three further candidate constraints exist in the source but are
  deliberately not asserted (see [`03a-operator-semantics.md`](./03a-operator-semantics.md)).
- **Bimodal** builds eleven model-structure and frame-axiom constraints (task nullity, converse,
  composition, plus world-enumeration/interval/lawfulness/abundance/uniqueness machinery over
  world histories); see [`11-theory-catalog.md`](./11-theory-catalog.md) for that theory's model
  shape.
