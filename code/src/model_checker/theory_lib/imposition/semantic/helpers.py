...
def safe_bitvec_as_long(bitvec: z3.BitVecRef) -> int:
    ...
    try:
        ...
        raise SemanticError(
            "Imposition helper function 'safe_bitvec_as_long' failed",
            theory="imposition",
            context={'function_name': 'safe_bitvec_as_long', 'bitvec': str(bitvec), 'bitvec_type': type(bitvec), 'original_error': str(e)},
...
def format_imposition_relation(state: z3.BitVecRef, world: z3.BitVecRef, outcome: z3.BitVecRef) -> str:
    ...
    try:
        ...
        raise SemanticError(
            "Imposition helper function 'format_imposition_relation' failed",
            theory="imposition",
            context={
                'function_name': 'format_imposition_relation',
                'state': str(state),
                'world': str(world),
                'outcome': str(outcome),
