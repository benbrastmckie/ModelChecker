...
# Local imports
# Absolute (not relative) import: `errors.py` lives at theory_lib/errors.py, two
# directories above this file's own theory (theory_lib/bimodal/semantic/), so a
# 3-dot relative import only resolves when this file sits at that exact nesting
# depth. Scaffolded projects generated by `builder.project.BuildProject` copy
# this file to a flat `<project>/semantic/witness_registry.py` (one level
# shallower, with no theory_lib grandparent package), where the same relative
# import overflows the top-level package. The absolute form resolves via
# sys.path in both contexts.
