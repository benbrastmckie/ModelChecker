...
Research agent specializing in formal mathematical logic for the Logos project. Handles modal logic, temporal logic, mereology, topology, and related mathematical foundations. Invoked by `skill-logic-research` via the forked subagent pattern. Uses hierarchical context loading from the logic domain index combined with codebase-first research strategy (LaTeX chapters + Lean source files).
...
- Spatial reasoning -> query for topics containing "mereology", "topology"
...
1. "What patterns exist in this codebase?"
   -> Glob for LaTeX chapters, Grep for concepts, Read key files

2. "What logic concepts are relevant?"
   -> Load domain context files (kripke, metalogic, proof-theory)

3. "How should proofs be structured?"
   -> Load process context files (modal-strategies, proof-construction)
...
5. "What external literature is relevant?"
   -> WebSearch for modal logic, mereology, topology, category theory
...
|------|---------|--------------|
| `01-Introduction.tex` | System overview | Background context |
| `02-ConstitutiveFoundation.tex` | Modal foundations | Modal logic research |
| `03-DynamicsFoundation.tex` | Temporal foundations | Temporal logic research |
| `04-Epistemic.tex` | Epistemic logic | Epistemic extensions |
| `05-Normative.tex` | Deontic logic | Normative extensions |
| `06-Spatial.tex` | Mereotopology | Spatial reasoning research |
| `08-Agential.tex` | Agency logic | Agent-oriented research |
| `09-Reflection.tex` | Self-reference | Meta-logical research |

### Lean Source (in `Theories/` and `Bimodal/` submodules)

| Module | Content | Research Use |
|--------|---------|--------------|
| `Bimodal.Syntax` | Formula types, parsing | Syntax research |
| `Bimodal.ProofSystem` | Axioms, derivation trees | Proof theory research |
| `Bimodal.Semantics` | Task frame semantics | Semantics research |
| `Bimodal.Metalogic` | Soundness and completeness | Metalogic research |
| `Bimodal.Theorems` | Perpetuity principles | Theorem patterns |

### Search Strategy for Codebase

```
1. Glob to find files: latex/subfiles/*.tex, Theories/**/*.lean
2. Grep for specific concepts: "Kripke", "modal", "temporal", "mereology", etc.
...
### Mereology/Topology Resources
- Stanford Encyclopedia (Mereology, Point-free Topology)
- Category theory for topological semantics
...
| Mereology | "mereology parthood relation formal", "general extensional mereology" |
| Topology | "point-free topology mereology connection", "closure algebra semantics" |
| Category theory | "category theory modal logic interpretation", "topos semantics modal" |
| Temporal logic | "LTL CTL temporal logic", "temporal frame conditions" |
...
2. **Identify research topic** from task description:
   ...
   - Metalogic -> load metalogic-concepts.md, verification-workflow.md
   - Proof theory -> load proof-theory-concepts.md, proof-construction.md
   - Spatial logic -> load spatial-domain.md
   - Task semantics -> load task-semantics.md
...
- `Glob` to find related files: `latex/subfiles/*.tex`, `Theories/**/*.lean`
- `Grep` for specific concepts in LaTeX and Lean files
...
- Focus queries on specific concepts (modal, mereology, etc.)
...
- Relevant patterns from codebase (LaTeX chapters, Lean modules)
...
context_gaps = [
  {
    ...
    "recommended_file": "relative path within .opencode/context/project/logic/",
...
**Target Location**: .opencode/context/project/logic/{directory}/{filename}
```
...
**Target File**: .opencode/context/project/logic/{path}
```
...
- Found existing modal semantics patterns in Bimodal.Semantics and 02-ConstitutiveFoundation.tex
...
- Found existing modal semantics patterns in Bimodal.Semantics and 02-ConstitutiveFoundation.tex
- Loaded kripke-semantics-overview.md and modal-proof-strategies.md context
- Identified 2 context gaps requiring documentation (canonical models, frame correspondence)
- Created 2 context extension tasks: #43 (canonical-models-domain.md), #44 (extend kripke-semantics-overview.md)
- Created report at specs/412_prove_modal_completeness/reports/research-001.md
- Metadata written for skill postflight
```
