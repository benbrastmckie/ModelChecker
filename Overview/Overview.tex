...
\begin{enumerate}
  ...
  \item[\it Parthood:] An $n$-state $s$ is \textit{part of} an $n$-state $t$ (i.e., $s\sqsubseteq t$) \textit{iff} $s_i\leq t_i$ for all $1\leq i\leq n$.
    \begin{itemize}
      \item Alternatively, an $n$-state $s$ is \textit{part of} an $n$-state $t$ \textit{iff} $(s \texttt{ OR } t) = t$  where: $11101010 \texttt{ OR } 00000100 = 11101110$. %the bitwise operation \texttt{OR} may be used to define parthood , though : $11101010 \texttt{ AND } 11111101 = 11101000$.
    \end{itemize}
