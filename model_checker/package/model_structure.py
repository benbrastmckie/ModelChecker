...
class ModelStructure:
    ...
    def __init__(
        ...
        find_all_constraints_func = make_constraints(
            verify, falsify, possible, assign, N, w
        )
        # TODO: replace prefix_sentences with ext_sub_sentences
        consts, sent_lets, input_prefix_sentences = find_all_constraints_func(
        ...
        self.extensional_subsentences = find_extensional_subsentences(
            input_prefix_sentences
        )
        self.counterfactual_subsentences = find_cf_subsentences(
            input_prefix_sentences
        )  # a list of prefix sentences (lists), not
    ...
    def rec_print(self, prop, current_world):
        ...
        # print(current_world)
        N = self.N
        if bitvec_to_substates(prop["comparison world"], N) != bitvec_to_substates(
            current_world, N
        ):
        ...
        if str(prop) in [str(atom) for atom in self.sentence_letters]:
            print(
                f"{prop} is {prop.truth_value_at(current_world)} in "
        ...
        print(
            f"{prop} is {prop.truth_value_at(current_world)} in "
            f"{bitvec_to_substates(current_world, N)} because:"
        )
        # print([bitvec_to_substates(ver, N) for ver in prop['verifiers']])
        # print(bitvec_to_substates(prop['comparison world'], N))
        # print(bitvec_to_substates(current_world, N))
        ...
        ), "{prop} is not a well-formed counterfactual since {left_subprop} is not extensional"
        self.rec_print(
            left_subprop, current_world
        )  # rec_print for extensional antecedent
        print(
...
class Proposition:
    def __init__(self, prefix_expr, model_structure, world):
        ...
        verifiers_in_model, falsifiers_in_model = (
